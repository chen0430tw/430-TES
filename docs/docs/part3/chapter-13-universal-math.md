...
**原版定义**：
$$\Delta d = 0 \quad \Leftrightarrow \quad \Gamma(\alpha^{\ast}) = \eta \cdot \frac{\partial \mathcal{E}}{\partial d}(\alpha^{\ast})$$
...
$$\frac{dd}{dt} = 0 \quad \Rightarrow \quad \Gamma(\alpha^{\ast}) = \eta \cdot \frac{\partial \mathcal{E}}{\partial d}\Big|_{\alpha^{\ast}}$$
...
2. 即维度增殖力 $\Gamma(\alpha^{\ast})$ 与维度压缩力 $\eta \frac{\partial \mathcal{E}}{\partial d}|_{\alpha^{\ast}}$ 平衡
3. 在该点附近，维度不再漂移
...
> **【原版定义】**
>
> 称 $\alpha^{\ast}$ 为序列 $\{\alpha_t\}$ 的**宇宙极限**，当且仅当：
>
> 1. **结构自洽（固定点）**：$T(\alpha^{\ast}) = \alpha^{\ast}$
> 2. **维度平衡（流为零）**：$\Delta d(\alpha^{\ast}) = 0$
> 3. **临界稳定（吸引子）**：在邻域内 $\mathcal{E}$ 下降
>
> 记作：$\lim_{t \to \infty} \alpha_t = \alpha^{\ast}$
...
1. **固定点**：系统不再变化 $\Rightarrow$ $T(\alpha^{\ast}) = \alpha^{\ast}$
...
宇宙数序列 $\{\alpha_n\}$ 的极限 $\alpha^{\ast}$ 定义为满足以下**三重条件**的唯一对象：

1. **结构自洽**：
   $$\mathcal{E}_{\text{GD}}(\alpha^{\ast}) = 0 \quad \Leftrightarrow \quad g(\alpha^{\ast}) = \Phi(d(\alpha^{\ast}))$$

2. **维度平衡**：
   $$\Gamma(\alpha^{\ast}) = \eta \cdot \frac{\partial \mathcal{E}}{\partial d}\Big|_{\alpha^{\ast}}$$
   即维度流达到临界不动点。

3. **临界稳定**：
   $$\forall \epsilon > 0, \exists N, \forall n > N: d_s(\alpha_n, \alpha^{\ast}) < \epsilon$$
   结构距离趋于零。
...
> **【原版定义】**
>
> 在极限点 $\alpha^{\ast}$ 邻域，取扰动 $\delta \alpha_t := \alpha_t \ominus \alpha^{\ast}$。
>
> **导数算子**为变换的线性化：
> $$DT(\alpha^{\ast}): \delta \alpha_t \mapsto \delta \alpha_{t+1}$$
>
> 其特征值决定：
> - $|\lambda| < 1$：稳定方向（扰动衰减）
> - $|\lambda| > 1$：不稳定方向（扰动放大）
...
2. 不动点满足 $z^{\ast} = T(z^{\ast}) \Rightarrow (z^{\ast})^2 = a + bi$
3. 分析稳定性：$T'(z^{\ast}) = -\frac{a+bi}{(z^{\ast})^2} = -1$
4. 边界稳定（$|T'| = 1$），需进一步分析
...
$$\lim_{n \to \infty} \alpha_n = \alpha^{\ast} \Leftrightarrow \begin{cases}
\mathcal{E}_{\text{GD}}(\alpha^{\ast}) = 0 & \text{(格度平衡)} \\
\Gamma(\alpha^{\ast}) = \eta \cdot \frac{\partial \mathcal{E}}{\partial d}\Big|_{\alpha^{\ast}} & \text{(临界稳定)} \\
d_s(\alpha_n, \alpha^{\ast}) \to 0 & \text{(结构收敛)}
\end{cases}$$
...
| 不动点 $f(U) = U$ | 临界固定点 $T(\alpha^{\ast}) = \alpha^{\ast}$ | B.6.1 ↔ A.3.1 |
